...
from Fuzzyfy import Fuzzyfier
from Defuzzyfy import Defuzzyfier
...
class FuzzyController:
    ...
    def __init__(self):
        ...
        self.Fuzzifier = Fuzzyfier(X1.copy(), X2.copy())
        self.Defuzzifier = Defuzzyfier(WHEEL.copy())

    def decide(self, left_dist, right_dist):
        # if left_dist>100 or right_dist>100:
        #     m=max(left_dist,right_dist)
        #     m-=100
        #     left_dist=max(0,left_dist-m)
        #     right_dist=max(0,right_dist-m)
        A_prime = self.Fuzzifier.Singeleton(left_dist, right_dist)
        B_primes = self.Engine.Zadeh(A_prime.copy())
        # plt.plot(WHEEL, B_primes[0])
        # plt.show()

        # plt.plot(WHEEL, B_primes[1])
        # plt.show()

        # plt.plot(WHEEL, B_primes[2])
        # plt.show()

        # plt.plot(WHEEL, B_primes[3])
        # plt.show()

        # plt.plot(WHEEL, B_primes[4])
        # plt.show()
