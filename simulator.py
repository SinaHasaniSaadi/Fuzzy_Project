...
TRACK = pygame.image.load(
    "./images/map.png"
).convert_alpha()  # convert to black and white
...
class Car:
    ...
    def crash_check(self):
        ...
        Returns:
            Bool: Returns True if the car hasn't carshed
        """
        for corner in self.corners:
            if TRACK.get_at(corner) == WHITE_COLOR:
                if (
                    1099 <= self.car_center[0]
                    and self.car_center[0] <= 1208
                    and self.car_center[1] > 516
                ):
                    pygame.mixer.pause()
...
def run():
    ...
    while True:
        ...
        # time.sleep(5)
        left_dist = math.sqrt(
            (car.car_center[0] - car.edge_points[0][0]) ** 2
            + (car.car_center[1] - car.edge_points[0][1]) ** 2
        )
        center_dist = math.sqrt(
            (car.car_center[0] - car.edge_points[1][0]) ** 2
            + (car.car_center[1] - car.edge_points[1][1]) ** 2
        )
        right_dist = math.sqrt(
            (car.car_center[0] - car.edge_points[2][0]) ** 2
            + (car.car_center[1] - car.edge_points[2][1]) ** 2
        )
        ...
        text = FONT.render(
            f"Left:{relative_left_dist:.2f} , Center:{center_dist:.2f} , Right:{relative_right_dist:.2f} ,    "
            + f"Output : {Command_wheel:.2f}",
            True,
            (0, 0, 0),
        )
